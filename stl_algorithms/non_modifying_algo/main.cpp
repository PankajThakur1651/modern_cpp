...
// Count, min, max , compare , Linear Search, Attribute, find, find_if,
// find_if_not, search_n
...
void count_usage() {
  ...
  std::cout << "Number 11 presence is " << presence_count << " times "
            << std::endl;
}
...
void linear_search() {
  ...
  auto find_if_not_itr =
      std::find_if_not(vec.begin(), vec.end(), [](int x) { return x <= 4; });
...
void all_none_any_of_algos() {
  auto condition =
      std::all_of(vec.begin(), vec.end(), [](int value) { return value >= 1; });

  std::cout << "condition is: " << condition << std::endl;

  condition = std::any_of(vec.begin(), vec.end(),
                          [](int value) { return value % 2 == 0; });

  std::cout << "any of modulus is 0: " << condition << std::endl;

  condition = std::none_of(vec.begin(), vec.end(),
                           [](int value) { return value % 2 == 0; });
