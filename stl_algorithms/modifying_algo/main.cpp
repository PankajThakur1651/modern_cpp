...
struct Test {
  ...
  Test(int a) : a_{a} { std::cout << __PRETTY_FUNCTION__ << std::endl; }

  ~Test() { std::cout << __PRETTY_FUNCTION__ << std::endl; }

  bool operator<(Test const &obj) const { return a_ < obj.a_; }
};
...
void copy_elements() {
  std::copy_if(vec.begin(), vec.end(), vec_2.begin(),
               [](int x) { return x > 2; });
...
void transform() {
  ...
  std::transform(local_vec.begin(), local_vec.end(),
                 std::back_inserter(local_vec_2), [](auto x) { return x - 1; });
