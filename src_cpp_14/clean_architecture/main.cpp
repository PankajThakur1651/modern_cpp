...
// Dependency Rule = your DB_operation depends only on an interface, not a
// concrete SQL class.
class DB_operation {
public:
  explicit DB_operation(IDataBase &db_operation) : db_operation_{db_operation} {
    std::cout << "DB_operation" << std::endl;
  }

  void create_table(std::string const &obj) { db_operation_.create_table(obj); }

  void delete_table(std::string const &obj) { db_operation_.delete_table(obj); }
