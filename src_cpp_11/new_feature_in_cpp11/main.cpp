...
public:
  ...
  Dog(int a) : age(a) { std::cout << "Default constructor " << std::endl; }
...
void after_cpp_11() {
  ...
  for (auto rev_itr = vec_of_ints.rbegin(); rev_itr != vec_of_ints.rend();
       rev_itr++) {
    std::cout << " " << *rev_itr << ", ";
  }
...
void foo(char * /*c*/) { std::cout << "foo(char*)" << std::endl; }

} // namespace nullptr_in_cpp11

namespace enum_classes {
enum class apple { RED, GREEN, BLUE };
...
void declare_array_from_constexpr_func() {
  ...
  std::cout << "size of array is " << sizeof(array) / sizeof(array[0])
            << std::endl;
}
...
template <typename func> void filter(func f, vector<int> &v) {
...
int main() {
  ...
  lambda_function_in_cpp::filter([](int x) { return x % 2 == 0; }, vec_of_ints);
}