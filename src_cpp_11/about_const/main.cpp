...
void const_basics() {
  ...
  ptr_1++;
  auto value_2 = 9;
  [[maybe_unused]] int *const ptr_2 =
      &value_2; // ptr_2 is const but not data it is pointing to

  // ptr_2++; is not allowed

  int const value_3 = 123;
  [[maybe_unused]] int const *const ptr_3 =
      &value_3; // // data is const , ptr_3 is also const
...
public:
  Big_array(std::vector<int> vec, int initial_value = 0)
      : vec_(std::move(vec)), access_counter_(initial_value) {}
